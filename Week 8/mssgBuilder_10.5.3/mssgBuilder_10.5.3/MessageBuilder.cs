...
namespace mssgBuilder_10._5._3
{
    ...
    public delegate string BufferedBuilderHandler(string buffer);
    public class MessageBuilder
    {
        public event BufferedBuilderHandler BufferEvent;
        ...
        public string GetMessage(string buffer)
        {
            int beginMark = bufferedData.IndexOf(messageBeginMarker);
            if (beginMark!= -1)
            {
                int endMark = bufferedData.IndexOf(messageEndMarker,beginMark);
                if (endMark != -1)
                {

                    if (BufferEvent != null)
                    {
                        string message = buffer.Substring(buffer.LastIndexOf(messageBeginMarker));
                        message = message.Remove(message.IndexOf(messageBeginMarker), 1);
                        message = message.Remove(message.LastIndexOf(messageEndMarker),1);
                        buffer = message;
                        BufferEvent(buffer);
                        return buffer;
                    }
